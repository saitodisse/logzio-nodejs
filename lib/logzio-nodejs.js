...
var LogzioLogger = function (options) {
    ...
    this.debugJson = options.debugJson || false;
    this.debugOutputFunction = options.debugOutputFunction || false;
...
LogzioLogger.prototype.sendAndClose = function(callback){
    ...
    this._debug({
        func: "sendAndClose",
        msg: "Sending last messages and closing..."
    });
...
LogzioLogger.prototype._timerSend = function() {
    if (this.messages.length > 0) {
        this._debug({
            func: "_timerSend",
            messagesLeft: this.messages,
            msg: 'Woke up and saw ' + this.messages.length + ' messages to send. Sending now...'
        });
        this._popMsgsAndSend();
    } else {
        this._debug({
...
LogzioLogger.prototype._sendMessagesUDP = function() {
    ...
    var udpSentCallback = function(err, bytes) {
        if (err) {
            this._debug({
                func: "_sendMessagesUDP",
                msg: 'Error while sending udp packets. err = ' + err
            });
    ...
    for (var i=0; i<messagesLength; i++) {
        ...
        this._debug({
            func: "_sendMessagesUDP",
            msg: 'Starting to send messages via udp.'
        });
...
LogzioLogger.prototype.log = function(msg) {
    ...
    if (this.messages.length >= this.bufferSize) {
        this._debug({
            func: "log",
            msg: 'Buffer is full - sending bulk'
        });
        this._popMsgsAndSend();
    }
};
...
LogzioLogger.prototype._popMsgsAndSend = function() {
    ...
    if (this.protocol === 'udp') {
        this._debug({
            func: "_popMsgsAndSend",
            msg: 'Sending messages via udp'
        });
    ...
    if (this.debug) {
        if (this.debugJson) {
            if (this.debugOutputFunction) {
                this.debugOutputFunction(msg)
            } else {
                console.log(msg);
            }
        } else {
            console.log('logzio-nodejs: ' + msg);
        }
    }
};
...
LogzioLogger.prototype._send = function(bulk) {
    ...
    function tryAgainIn(sleepTimeMs) {
        mythis._debug({
            func: "_send",
            bulkId: bulk.id,
            msg: 'Bulk #' + bulk.id + ' - Trying again in ' + sleepTimeMs + '[ms], attempt no. ' + bulk.attemptNumber
        });
        setTimeout(function() {
            mythis._send(bulk);
        }, sleepTimeMs);
            ...
            if (err) {
                ...
                if (err.code === 'ETIMEDOUT' || err.code === 'ECONNRESET' || err.code === 'ESOCKETTIMEDOUT' || err.code === 'ECONNABORTED') {
                    if (bulk.attemptNumber >= mythis.numberOfRetries) {
                        ...
                        mythis._debug({
                            func: "_send",
                            bulkId: bulk.id,
                            msg: 'Bulk #' + bulk.id + ' - failed on error: ' + err
                        });
                ...
                if (responseCode !== '200') {
                    ...
                    mythis._debug({
                        func: "_send_success",
                        bulkId: bulk.id,
                        msg: 'Bulk #' + bulk.id + ' - sent successfully'
                    });
                    callback();
                }
            }
